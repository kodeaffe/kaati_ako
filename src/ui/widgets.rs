...
impl AboutDialog {
    ...
    pub fn show(parent: &gtk::ApplicationWindow) {
        ...
        let licence = match fs::read_to_string(licence_path) {
            Ok(licence) => licence,
            Err(err) => {
                ErrorDialog::show(parent, &format!("{}: {}", err, licence_path));
                return;
            },
        };
...
impl AddCardDialog {
    ...
    fn build(parent: &gtk::ApplicationWindow) -> Result<gtk::Dialog, DatabaseError> {
        ...
        let grid = AddCardDialog::build_grid(&languages)?;
        ...
        dialog.connect_response(glib::clone!(@weak parent => move |_, response_type| {
            if response_type == gtk::ResponseType::Accept {
                AddCardDialog::response_accept(&parent, &conn, &languages, &grid);
            }
        }));
        Ok(dialog)
    }
    ...
    /// Show the dialog
    pub fn show(parent: &gtk::ApplicationWindow) {
        let dialog = match AddCardDialog::build(parent) {
            Ok(dialog) => dialog,
            Err(err) => {
                ErrorDialog::show(&parent, &err.to_string());
                return;
